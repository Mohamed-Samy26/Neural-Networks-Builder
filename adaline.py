...
class Adaline :
    ...
    def plot_decision_boundary(self, X, Y, labels=['Class 1', 'Class -1'], feature_names=['Feature 1', 'Feature 2']):
            X_np = X.to_numpy() if isinstance(X, pd.DataFrame) else X
            Y_np = Y.to_numpy() if isinstance(Y, pd.DataFrame) else Y

            # Plot the decision boundary
            plt.scatter(X_np[Y_np == 1][:, 0], X_np[Y_np == 1][:, 1], color='blue', label=labels[0])
            plt.scatter(X_np[Y_np == -1][:, 0], X_np[Y_np == -1][:, 1], color='red', label=labels[1])
    ...
    def plot_confusion_matrix(self, X, Y, labels=['Class 1', 'Class -1']):
        ...
        plt.xticks(ticks=[0, 1], labels=['Predicted '+labels[0], 'Predicted '+labels[1]])
