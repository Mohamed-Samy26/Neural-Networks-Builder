...
features = [(0, "Area"), (1, "Perimeter"), (2, "MajorAxisLength"), (3, "MinorAxisLength"), (4, "roundness")]
...
def button_callback():
    ...
    if not learning_rate_value or not is_number(learning_rate_value):
        CTkMessagebox(title="Error", message="Please enter a valid Learning Rate (numeric value)", icon="cancel")
        return

        # Check if epochs is None or empty
    if not epochs_value or not epochs_value.isdigit():
        CTkMessagebox(title="Error", message="Please enter a valid number of Epochs (positive integer)", icon="cancel")
        return

        # Check if mse_threshold is None or empty
    if not mse_threshold_value or not is_number(mse_threshold_value):
        CTkMessagebox(title="Error", message="Please enter a valid MSE Threshold (numeric value)", icon="cancel")
        return
...
customtkinter.CTkLabel(master=frame_2, text="First Feature", font=("Arial Bold", 16)).grid(column=0, row=0, sticky='w',
                                                                                           pady=(20, 5), padx=(400, 0))
first_feature = customtkinter.CTkComboBox(frame_2, values=features, command=pickFeature1, width=170)
first_feature.grid(column=0, row=1, sticky='w', pady=(10, 20), padx=(400, 0))
first_feature.set("Select First Feature")

customtkinter.CTkLabel(master=frame_2, text="Second Feature", font=("Arial Bold", 16)).grid(column=0, row=2, sticky='w',
                                                                                            pady=(20, 5), padx=(400, 0))

second_feature = customtkinter.CTkComboBox(frame_2, values=features, command=pickFeature2, width=170)
second_feature.grid(column=0, row=3, sticky='w', pady=(10, 20), padx=(400, 0))
second_feature.set("Select Second Feature")

customtkinter.CTkLabel(master=frame_2, text="Classes", font=("Arial Bold", 16)).grid(column=1, row=0, sticky='w',
                                                                                           pady=(20, 5), padx=(130, 0))

classes = customtkinter.CTkComboBox(frame_2, values=["BOMBAY & CALI", "CALI & SIRA", "SIRA & BOMBAY"])
classes.grid(column=1, row=1, sticky='w', pady=(10, 20), padx=(130, 0))
classes.set("Select Classes")



customtkinter.CTkLabel(master=frame_2, text="Algorithm", font=("Arial Bold", 20)).grid(column=1, row=2, sticky='w',
                                                                                           pady=(20, 0), padx=(130, 0))

isPerceptron = customtkinter.IntVar(value=0)

radiobutton_1 = customtkinter.CTkRadioButton(frame_2, variable=isPerceptron, text="Perceptron", value=0)
radiobutton_1.grid(column=1, row=3, sticky='w', pady=(20, 20), padx=(130, 0))

radiobutton_2 = customtkinter.CTkRadioButton(frame_2, variable=isPerceptron, text="Adaline", value=1)
...
customtkinter.CTkLabel(master=frame_1, text="Learning Rate", font=("Arial Bold", 16), justify="left").pack(anchor="w",
                                                                                                           pady=(20, 7),
                                                                                                           padx=(25))

learning_rate = customtkinter.CTkEntry(master=frame_1, placeholder_text="Please enter a float", fg_color="#f9fdff",
                                       font=("Arial Bold", 16), border_width=0, corner_radius=5, height=50 , text_color="#000000")
learning_rate.pack(fill="x", anchor="w", padx=(25))

customtkinter.CTkLabel(master=frame_1, text="Number of Epochs", font=("Arial Bold", 16), justify="left").pack(
    anchor="w", pady=(30, 7), padx=(25))

epochs = customtkinter.CTkEntry(master=frame_1, placeholder_text="Please enter an integer", fg_color="#f9fdff", font=("Arial Bold", 16),
                                border_width=0, corner_radius=5, height=50)
epochs.pack(fill="x", anchor="w", padx=(25))

customtkinter.CTkLabel(master=frame_1, text="MSE Threshold", font=("Arial Bold", 16), justify="left").pack(anchor="w",
                                                                                                           pady=(30, 7),
                                                                                                           padx=(25))

mse_threshold = customtkinter.CTkEntry(master=frame_1, placeholder_text="Please enter a float", fg_color="#f9fdff",
                                       font=("Arial Bold", 16), border_width=0, corner_radius=5, height=50)
mse_threshold.pack(fill="x", anchor="w", padx=(25), pady=(0, 25))


Classify = customtkinter.CTkButton(app, command=button_callback, font=("Arial Bold", 20) , text="Classify" , height=100)
